...
- [ ] 06-history-ring-buffers — Fixed-capacity `HistoryBuffer<T>` ring buffer in Core + per-process/per-sensor history retention in the engine, sparkline point downsampling; accept: tests cover wrap-around, capacity, downsample output.
